  ...
  const getDemoTrips = (transportType?: TransportTypeEnum) => {
    ...
    const allDemoTrips: Trip[] = [
      // 1. TRANSCO - Kinshasa → Lubumbashi (Bus longue distance)
      {
        id: 1,
        route_id: 1,
        departure_time: new Date(tomorrow.getTime()).setHours(8, 0, 0, 0).toString(),
        arrival_time: new Date(tomorrow.getTime()).setHours(20, 0, 0, 0).toString(),
        available_seats: 45,
      ...
      // 2. TRANSCO - Kinshasa → Matadi (Bus régional)
      {
        id: 2,
        route_id: 2,
departure_time: new Date(tomorrow.getTime()).setHours(10, 30, 0, 0).toString(),
        arrival_time: new Date(tomorrow.getTime()).setHours(15, 30, 0, 0).toString(),
        available_seats: 12,
      ...
      // 3. TRAIN - Kinshasa → Goma (Train longue distance)
      {
        id: 3,
        route_id: 3,
departure_time: new Date(dayAfter.getTime()).setHours(7, 0, 0, 0).toString(),
        arrival_time: new Date(dayAfter.getTime()).setHours(19, 0, 0, 0).toString(),
        available_seats: 30,
      ...
      // 4. FLUVIAL - Kinshasa → Kisangani (Bateau)
      {
        id: 4,
        route_id: 4,
departure_time: new Date(tomorrow.getTime()).setHours(16, 0, 0, 0).toString(),
        arrival_time: new Date(tomorrow.getTime()).setHours(20, 0, 0, 0).toString(),
        available_seats: 28,
      ...
      // 5. PRIVATE - Goma → Bukavu (Transport privé)
      {
        id: 5,
        route_id: 5,
departure_time: new Date(dayAfter.getTime()).setHours(9, 15, 0, 0).toString(),
        arrival_time: new Date(dayAfter.getTime()).setHours(12, 15, 0, 0).toString(),
        available_seats: 8,
      ...
      // 6. TRANSCO - Lubumbashi → Kolwezi (Bus minier)
      {
        id: 6,
        route_id: 6,
departure_time: new Date(tomorrow.getTime()).setHours(6, 0, 0, 0).toString(),
        arrival_time: new Date(tomorrow.getTime()).setHours(10, 0, 0, 0).toString(),
        available_seats: 35,
      ...
      // 7. TRAIN - Lubumbashi → Kamina (Train régional)
      {
        id: 7,
        route_id: 7,
departure_time: new Date(dayAfter.getTime()).setHours(14, 0, 0, 0).toString(),
        arrival_time: new Date(dayAfter.getTime()).setHours(20, 0, 0, 0).toString(),
        available_seats: 22,
      ...
      // 8. FLUVIAL - Kisangani → Mbandaka (Bateau fluvial)
      {
        id: 8,
        route_id: 8,
departure_time: new Date(tomorrow.getTime()).setHours(8, 30, 0, 0).toString(),
        arrival_time: new Date(tomorrow.getTime()).setHours(18, 30, 0, 0).toString(),
        available_seats: 15,
          ...
          // 9. PRIVATE - Kinshasa → Bandundu (Minibus privé)
      {
        id: 9,
        route_id: 9,
        departure_time: new Date(tomorrow.getTime()).setHours(12, 0, 0, 0).toString(),
        arrival_time: new Date(tomorrow.getTime()).setHours(16, 30, 0, 0).toString(),
        available_seats: 6,
  ...
  const searchTripsByTransportType = async (transportType: TransportTypeEnum) => {
    ...
    setTimeout(() => {
      let demoTrips = getDemoTrips(transportType);
      // Filtrer par ville de départ/arrivée si renseignées
      const dep = (bookingData.departureCity || '').trim().toLowerCase();
      const arr = (bookingData.arrivalCity || '').trim().toLowerCase();
      if (dep) {
        demoTrips = demoTrips.filter(t => t.route?.departure_city?.toLowerCase().includes(dep));
      }
      if (arr) {
        demoTrips = demoTrips.filter(t => t.route?.arrival_city?.toLowerCase().includes(arr));
      }
      // Filtrer sur la date si fournie (comparaison sur yyyy-mm-dd)
      if (bookingData.departureDate && bookingData.departureDate.length === 10) {
        const target = bookingData.departureDate;
        demoTrips = demoTrips.filter(t => {
          const d = new Date(typeof t.departure_time === 'string' ? parseInt(t.departure_time, 10) : (t as any).departure_time);
          const ymd = ymdLocal(d);
          return ymd === target;
        });
      }
      // Tolérance: si 0 résultat avec date, réessayer sans date
      if (demoTrips.length === 0 && bookingData.departureDate) {
        let retry = getDemoTrips(transportType);
        if (dep) retry = retry.filter(t => t.route?.departure_city?.toLowerCase().includes(dep));
        if (arr) retry = retry.filter(t => t.route?.arrival_city?.toLowerCase().includes(arr));
        demoTrips = retry;
      }
      // Si aucun critère n'est fourni, afficher tout
      if (!dep && !arr && !bookingData.departureDate) {
        demoTrips = getDemoTrips(transportType);
      }
      setTrips(demoTrips);
      setLoading(false);
    }, 200);
  ...
  const formatTime = (timestamp: string | number) => {
    try {
      ...
      return new Date(timestampNum).toLocaleTimeString('fr-FR', {
        hour: '2-digit',
        minute: '2-digit',
      });
    } catch (error) {
  ...
  const formatDuration = (minutes: number) => {
    ...
    return `${hours}h${mins > 0 ? (mins < 10 ? '0' + mins : mins) : '00'}`;
  };
  ...
  const handleSearchTrips = async () => {
    ...
    setTimeout(() => {
      ...
      const arr = (bookingData.arrivalCity || '').trim().toLowerCase();
      if (dep) {
        filteredTrips = filteredTrips.filter(t => t.route?.departure_city?.toLowerCase().includes(dep));
      }
      if (arr) {
        filteredTrips = filteredTrips.filter(t => t.route?.arrival_city?.toLowerCase().includes(arr));
      }
      // Filtrage date (yyyy-mm-dd)
      if (bookingData.departureDate && bookingData.departureDate.length === 10) {
        const target = bookingData.departureDate;
      ...
      // Tolérance: si 0 résultat avec date, réessayer sans la date
      if (filteredTrips.length === 0 && bookingData.departureDate) {
        let retry = getDemoTrips(selectedTransport || undefined);
        if (dep) retry = retry.filter(t => t.route?.departure_city?.toLowerCase().includes(dep));
        if (arr) retry = retry.filter(t => t.route?.arrival_city?.toLowerCase().includes(arr));
        filteredTrips = retry;
      }
      // Si aucun critère, afficher tous les trajets mock
      if (!dep && !arr && !bookingData.departureDate && !selectedTransport) {
        filteredTrips = getDemoTrips();
      }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center space-x-4">
              {session ? (
                <>
                  <span className="text-emerald-100">Bonjour</span>
                  <button
      ...
      <section className="bg-gradient-to-r from-violet-600 via-purple-600 to-indigo-600 text-white py-12 relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="bg-white rounded-2xl shadow-2xl p-6 md:p-8 animate-fadeIn">
            ...
            <form
              onSubmit={(e) => { e.preventDefault(); handleSearchTrips(); }}
            >
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                  <label htmlFor="departureCity" className="block text-sm font-medium text-slate-700 mb-2">
                ...
                <div>
                  <label htmlFor="arrivalCity" className="block text-sm font-medium text-slate-700 mb-2">
                    <MapPin className="w-4 h-4 inline mr-1" />
                    Arrivée
                  </label>
                  <input
